    ...
    )
{
    ...
    _MSPoolCatPrint (fmt, args, &spc, _SPrint);
    ...
    )
{
    POOL_PRINT          spc;
    ZeroMem (&spc, sizeof(spc));
    _MSPoolCatPrint (fmt, args, &spc, _PoolPrint);
    return spc.str;
}
...
{
    return _MSIPrint ((UINTN) -1, (UINTN) -1, ST->ConOut, fmt, NULL, args);
}
...
)
{
    ...
    Len = MSUnicodeVSPrint(UnicodeStr, StrSize, UnicodeFmt, args);
    FreePool(UnicodeFmt);
    ...
    )
{
    ...
    // pad item
    for (i=Len; i < Item->Width; i++) {
        MS_PPUTC (ps, Item->Pad);
    }

    // add the item
    Item->Item.Index=0;
    while (Item->Item.Index < Len) {
        MS_PPUTC (ps, PGETC(&Item->Item));
    }
    ...
    )
{
    ...
    while ((c = PGETC(&ps->fmt))) {
        ...
        while ((c = PGETC(&ps->fmt))) {
            ...
            case 'h':
                MS_PSETATTR(ps, ps->AttrHighlight);
                break;
